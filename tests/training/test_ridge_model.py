...
def test_build_ridge_model_returns_pipeline() -> None:
    model = build_ridge_model()

    assert isinstance(model, Pipeline)


def test_ridge_model_fits_successfully() -> None:
...
def test_ridge_model_predicts_correct_number_of_rows() -> None:
    frame = make_frame_with_price()
...
def test_ridge_model_predictions_are_not_all_identical() -> None:
    frame = make_frame_with_price()
...
def test_ridge_model_alpha_zero_raises_value_error() -> None:
    with pytest.raises(ValueError, match="alpha must be greater than zero."):
        build_ridge_model(alpha=0)


def test_ridge_model_does_not_mutate_input() -> None:
    frame = make_frame_with_price()
